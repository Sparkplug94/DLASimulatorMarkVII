...
mu_y = 0e-9; %beam centroid relative to channel, m
...
n1 = 15; %20 DLA periods
eps = 100e6; %Peak Acceleration Gradient, in MeV/m 
theta_r = 0; %relative laser phase
rn = 0;

phaseSpace = remove(phaseSpace,'y',200e-9);
phaseSpace = remove(phaseSpace,'x',200e-9);
...
for i = 1:n1
    ...
    phaseSpace = remove(phaseSpace,'y',200e-9);
    phaseSpace = remove(phaseSpace,'x',400e-9);
    deltaMin = 1/phaseSpace.gamma_s - 1;
    phaseSpace = remove(phaseSpace,'delta',abs(deltaMin));
    phaseSpace = DLAUpdate( phaseSpace, eps, theta_r, rn, sigma_tau_las);
    if sum(sum(imag(phaseSpace.dist))) ~= 0
        disp(['Complex Values Detected, cell ' num2str(i)])
    end
    phaseSpace = validate(phaseSpace);
end

period = gamma2beta(phaseSpace.gamma_s)*lam0;
N = 15.75;
phaseSpace = driftUpdate(phaseSpace,N*period);
...
for i = 1:n1
    ...
    deltaMin = 1/phaseSpace.gamma_s - 1;
    phaseSpace = remove(phaseSpace,'delta',abs(deltaMin));
    phaseSpace = DLAUpdate( phaseSpace, eps, pi, rn, sigma_tau_las);
    if sum(sum(imag(phaseSpace.dist))) ~= 0
        disp(['Complex Values Detected, cell ' num2str(i)])
    end
    phaseSpace = validate(phaseSpace);
...
ymax = 200e-9; %for example, let's say my channel width is 500 nm
...
subplot(2,3,6)
plotPhaseSpace(phaseSpace.dist, 'xy')
