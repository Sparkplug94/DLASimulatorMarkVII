...
%peak gradient
eps0 = 200e6; %REAL NUMBER - initial eps_0

%structure constant as a function of beta (analytical approximation based on lumerical
%sim)
cs = @(b) -2.8675*b^2+4.4780*b-1.2062;
cs0 = cs(beta0);
E0 = eps0/cs0; %incident electric field
...
for i = 1:cells
    ...
    switch opt
        case 'DLA'
            ...
            b = gamma2beta(phaseSpace.gamma_s);
            eps = cs(b)*E0;
            %disp(eps)
            %DLA kick
...
writeLattice(latticeDesign,'run_v6_latticeDesign_matlabExport');
